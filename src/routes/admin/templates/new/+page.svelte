...
<div class="container mx-auto max-w-3xl p-6 space-y-6">
	...
	<aside class="card preset-tonal-primary p-4 space-y-1">
		...
		<p class="text-sm">
			...
			<code>Student</code> / a strong build password (do not commit it). Not sure where to start?
					...
					{#if req.unattend_mode !== 'manual'}
						<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
							...
							<label class="label">
								...
								<span class="text-xs text-surface-500 mt-1 block">
									Set a strong build password (do not commit it). Don't leave this blank.
								</span>
							</label>
							<label class="label">
							...
							<label class="label">
								...
								<input class="input" type="text" bind:value={unattendAptProxy} placeholder="http://192.0.2.20:3142" />
								<span class="text-xs text-surface-500 mt-1 block">
									Linux only, optional. Add <code>http://192.0.2.20:3142</code> for faster package downloads.
								</span>
							</label>
						</div>
						<label class="label">
